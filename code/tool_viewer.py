...
class FileExplorer(QMainWindow):
    def __init__(self):
        ...
        self.label.setAlignment(Qt.AlignLeft)  # ← ここを左寄せに変更
        ...
        #splitter.addWidget(self.back_button)
        ...
        # ストレッチ設定（ラベルは固定）
        splitter.setStretchFactor(0, 0)  # ツリーアイテム
    ...
    def on_item_clicked(self, index):
        '''
        if(self.tree_item_clickevnt == index):
            self.tree_item_clickevnt = None
            self.on_item_double_clicked(index)
        '''
        #win = index.data(Qt.UserRole)
        ...
        if hwnd:
            try:
                ...
                #print(f"[EXE] {win.exe_path}")
                #print(f"[TARGET] {win.target_path if win.target_path else '(対象ファイルなし)'}")
    ...
    def item_context_menu(self,position):#編集
        ...
        if not index.isValid():
                return
        #path = self.models.filePath(index)
        menu = QMenu()
    ...
    def open_register_dialog(self,path):
        ...
        #path = r"C:\Users\admin\OneDrive\ダウンロード"
